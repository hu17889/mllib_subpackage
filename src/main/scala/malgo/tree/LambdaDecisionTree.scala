...
import org.apache.spark.mllib.tree.LambdaRandomForest.NodeIndexInfo
import org.apache.spark.mllib.tree.configuration.LambdaStrategy
...
@Since("1.0.0")
@Experimental
class LambdaDecisionTree @Since("1.0.0") (private val strategy: LambdaStrategy)
   ...
   * @return LambdaDecisionTreeModel that can be used for prediction
   */
  @Since("1.2.0")
  def run(input: RDD[LambdaLabeledPoint]): LambdaDecisionTreeModel = {
    // Note: random seed will not be used since numTrees = 1.
    val rf = new LambdaRandomForest(strategy, numTrees = 1, featureSubsetStrategy = "all", seed = 0)
...
@Since("1.0.0")
object LambdaDecisionTree extends Serializable with Logging {
   ...
   */
 @Since("1.0.0")
  def train(input: RDD[LambdaLabeledPoint], strategy: LambdaStrategy): LambdaDecisionTreeModel = {
    new LambdaDecisionTree(strategy).run(input)
  }
  ...
  def train(
      ...
      algo: Algo,
      impurity: Impurity,
      maxDepth: Int): LambdaDecisionTreeModel = {
    val strategy = new LambdaStrategy(algo, impurity, maxDepth)
    new LambdaDecisionTree(strategy).run(input)
  }
  ...
  def train(
      ...
      impurity: Impurity,
      maxDepth: Int,
      numClasses: Int): LambdaDecisionTreeModel = {
    val strategy = new LambdaStrategy(algo, impurity, maxDepth, numClasses)
    new LambdaDecisionTree(strategy).run(input)
  }
  ...
  def train(
      ...
      categoricalFeaturesInfo: Map[Int, Int]): LambdaDecisionTreeModel = {
    val strategy = new LambdaStrategy(algo, impurity, maxDepth, numClasses, maxBins,
  ...
  def trainClassifier(
      ...
      impurity: String,
      maxDepth: Int,
      maxBins: Int): LambdaDecisionTreeModel = {
  ...
  private def predictNodeIndex(
    ...
    if (node.isLeaf || node.split.isEmpty) {
      ...
      if (node.leftNode.isEmpty || node.rightNode.isEmpty) {
        ...
        if (splitLeft) {
          MNode.leftChildIndex(node.id)
        } else {
          MNode.rightChildIndex(node.id)
        }
      } else {
        if (splitLeft) {
  ...
  private def mixedBinSeqOp(
      agg: LambdaDTStatsAggregator,
      treePoint: LambdaTreePoint,
      ...
      if (unorderedFeatures.contains(featureIndex)) {
        ...
        while (splitIndex < numSplits) {
          if (splits(featureIndex)(splitIndex).categories.contains(featureValue)) {
            agg.featureUpdate(leftNodeFeatureOffset, splitIndex, treePoint.label,treePoint,
              instanceWeight)
          } else {
            agg.featureUpdate(rightNodeFeatureOffset, splitIndex, treePoint.label,treePoint,
              instanceWeight)
          }
          splitIndex += 1
        }
        ...
        agg.update(featureIndexIdx, binIndex, treePoint.label,treePoint, instanceWeight)
      }
      featureIndexIdx += 1
    }
  }
  ...
  private def orderedBinSeqOp(
    ...
    if (featuresForNode.nonEmpty) {
      ...
      while (featureIndexIdx < featuresForNode.get.size) {
        ...
        agg.update(featureIndexIdx, binIndex, label,treePoint, instanceWeight)
        featureIndexIdx += 1
      }
    } else {
      // Use all features
      ...
      while (featureIndex < numFeatures) {
        val binIndex = treePoint.binnedFeatures(featureIndex)
        agg.update(featureIndex, binIndex, label,treePoint, instanceWeight)
        featureIndex += 1
      }
    }
  }
   ...
   */
  def findBestSplits(
      input: RDD[BaggedPoint[LambdaTreePoint]],
      metadata: LambdaDecisionTreeMetadata,
      topNodes: Array[MNode],
      nodesForGroup: Map[Int, Array[MNode]],
      treeToNodeToIndexInfo: Map[Int, Map[Int, NodeIndexInfo]],
      splits: Array[Array[Split]],
      bins: Array[Array[Bin]],
      nodeQueue: mutable.Queue[(Int, MNode)],
      timer: TimeTracker = new TimeTracker,
      nodeIdCache: Option[LambdaNodeIdCache] = None): Unit = {
    ...
    def nodeBinSeqOp(
        treeIndex: Int,
        nodeInfo: LambdaRandomForest.NodeIndexInfo,
        agg: Array[LambdaDTStatsAggregator],
        baggedPoint: BaggedPoint[LambdaTreePoint]): Unit = {
      if (nodeInfo != null) {
    ...
    def binSeqOp(
        agg: Array[LambdaDTStatsAggregator],
        baggedPoint: BaggedPoint[LambdaTreePoint]): Array[LambdaDTStatsAggregator] = {
    ...
    def binSeqOpWithNodeIdCache(
        agg: Array[LambdaDTStatsAggregator],
        dataPoint: (BaggedPoint[LambdaTreePoint], Array[Int])): Array[LambdaDTStatsAggregator] = {
      treeToNodeToIndexInfo.foreach { case (treeIndex, nodeIndexToInfo) =>
    ...
    val nodes = new Array[MNode](numNodes)
    ...
    val partitionAggregates : RDD[(Int, LambdaDTStatsAggregator)] = if (nodeIdCache.nonEmpty) {
      input.zip(nodeIdCache.get.nodeIdsForInstances).mapPartitions { points =>
        ...
        val nodeStatsAggregators = Array.tabulate(numNodes) { nodeIndex =>
          ...
          new LambdaDTStatsAggregator(metadata, featuresForNode)
        }
    ...
    //partitionAggregates.map{case x =>(x._1,x._2.allStats.mkString("_"))}.saveAsTextFile("/home/hdp_teu_search/hucong/output/agg"+System.nanoTime().toString)
    ...
    nodesForGroup.foreach { case (treeIndex, nodesForTree) =>
      nodesForTree.foreach { node =>
        ...
        val isLeaf = (stats.gain <= 0) || (MNode.indexToLevel(nodeIndex) == metadata.maxDepth)
        ...
        if (!isLeaf) {
          ...
          val childIsLeaf = (MNode.indexToLevel(nodeIndex) + 1) == metadata.maxDepth
          val leftChildIsLeaf = childIsLeaf || (stats.leftImpurity == 0.0)
          val rightChildIsLeaf = childIsLeaf || (stats.rightImpurity == 0.0)
          node.leftNode = Some(MNode(MNode.leftChildIndex(nodeIndex),
            stats.leftPredict, stats.leftImpurity, leftChildIsLeaf))
          node.rightNode = Some(MNode(MNode.rightChildIndex(nodeIndex),
  ...
  private def binsToBestSplit(
      binAggregates: LambdaDTStatsAggregator,
      splits: Array[Array[Split]],
      featuresForNode: Option[Array[Int]],
      node: MNode): (Split, InformationGainStats, Predict) = {

    // calculate predict and impurity if current node is top node
    val level = MNode.indexToLevel(node.id)
  ...
  protected[tree] def findSplitsBins(
      input: RDD[LambdaLabeledPoint],
      metadata: LambdaDecisionTreeMetadata): (Array[Array[Split]], Array[Array[Bin]]) = {
    ...
    val sampledInput = if (hasContinuousFeatures) {
      ...
      new Array[LambdaLabeledPoint](0)
    }
   ...
   *       The `numSplits` attribute in the `LambdaDecisionTreeMetadata` class will be set accordingly.
