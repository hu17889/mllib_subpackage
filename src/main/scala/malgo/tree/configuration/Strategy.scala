...
import org.apache.spark.mllib.tree.impurity.{Variance, Entropy, Gini, Impurity,LambdaVariance}
...
@Since("1.0.0")
@Experimental
class LambdaStrategy @Since("1.3.0") (
  ...
  /**
   * Java-friendly constructor for [[org.apache.spark.mllib.tree.configuration.LambdaStrategy]]
   */
  @Since("1.1.0")
  def this(
      algo: Algo,
    ...
    algo match {
      case Classification =>
        require(numClasses >= 2,
          s"DecisionTree LambdaStrategy for Classification must have numClasses >= 2," +
          s" but numClasses = $numClasses.")
        require(Set(Gini, Entropy).contains(impurity),
          s"DecisionTree LambdaStrategy given invalid impurity for Classification: $impurity." +
          s"  Valid settings: Gini, Entropy")
      case Regression =>
        require(Set(Variance, LambdaVariance).contains(impurity),
          s"DecisionTree LambdaStrategy given invalid impurity for Regression: $impurity." +
          s"  Valid settings: Variance")
      case _ =>
        throw new IllegalArgumentException(
          s"DecisionTree LambdaStrategy given invalid algo parameter: $algo." +
          s"  Valid settings are: Classification, Regression.")
    }
    require(maxDepth >= 0, s"DecisionTree LambdaStrategy given invalid maxDepth parameter: $maxDepth." +
      s"  Valid values are integers >= 0.")
    require(maxBins >= 2, s"DecisionTree LambdaStrategy given invalid maxBins parameter: $maxBins." +
      s"  Valid values are integers >= 2.")
    require(minInstancesPerNode >= 1,
      s"DecisionTree LambdaStrategy requires minInstancesPerNode >= 1 but was given $minInstancesPerNode")
    require(maxMemoryInMB <= 10240,
      s"DecisionTree LambdaStrategy requires maxMemoryInMB <= 10240, but was given $maxMemoryInMB")
    require(subsamplingRate > 0 && subsamplingRate <= 1,
      s"DecisionTree LambdaStrategy requires subsamplingRate <=1 and >0, but was given " +
      s"$subsamplingRate")
  }
   ...
   */
  @Since("1.2.0")
  def copy: LambdaStrategy = {
    new LambdaStrategy(algo, impurity, maxDepth, numClasses, maxBins,
...
object LambdaStrategy {
   ...
   */
  @Since("1.2.0")
  def defaultStrategy(algo: String): LambdaStrategy = {
   ...
   */
  @Since("1.3.0")
  def defaultStrategy(algo: Algo): LambdaStrategy = algo match {
    case Algo.Classification =>
      new LambdaStrategy(algo = Classification, impurity = Gini, maxDepth = 10,
        numClasses = 2)
    case Algo.Regression =>
      new LambdaStrategy(algo = Regression, impurity = LambdaVariance, maxDepth = 10,
        numClasses = 0)
  }

  @deprecated("Use LambdaStrategy.defaultStrategy instead.", "1.5.0")
  @Since("1.2.0")
  def defaultStategy(algo: Algo): LambdaStrategy = defaultStrategy(algo)
