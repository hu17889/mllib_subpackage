 ...
 */
class LambdaTreePoint(val label: Double,val lambda: Double, val w: Double, val binnedFeatures: Array[Int])
  extends Serializable {
  var qid:Int=0
  var pos:Int=0
}

object LambdaTreePoint {
  ...
  def convertToTreeRDD(
      ...
      metadata: LambdaDecisionTreeMetadata): RDD[LambdaTreePoint] = {
    ...
    input.map { x =>
      labeledPointToTreePoint(x, bins, featureArity)
    }
  }
  ...
  private def labeledPointToTreePoint(
      labeledPoint: LambdaLabeledPoint,
      bins: Array[Array[Bin]],
      featureArity: Array[Int]): LambdaTreePoint = {
    ...
    new LambdaTreePoint(labeledPoint.label,labeledPoint.lamda,labeledPoint.w, arr)
  }
