 ...
 */
private[spark] class LambdaDTStatsAggregator(
    val metadata: LambdaDecisionTreeMetadata,
    featureSubset: Option[Array[Int]]) extends Serializable {

  /**
   * [[LambdaImpurityAggregator]] instance specifying the impurity type.
   */
  val impurityAggregator: LambdaImpurityAggregator = metadata.impurity match {
    //case Gini => new GiniAggregator(metadata.numClasses)
    //case Entropy => new EntropyAggregator(metadata.numClasses)
    //case Variance => new VarianceAggregator()
    case LambdaVariance => new LambdaVarianceAggregator()
   ...
   */
  def update(featureIndex: Int, binIndex: Int, label: Double, point: LambdaTreePoint, instanceWeight: Double): Unit = {
    val i = featureOffsets(featureIndex) + binIndex * statsSize
    impurityAggregator.update(allStats, i, label,point, instanceWeight)
  }
   ...
   */
  def merge(other: LambdaDTStatsAggregator): LambdaDTStatsAggregator = {
    require(allStatsSize == other.allStatsSize,
      s"LambdaDTStatsAggregator.merge requires that both aggregators have the same length stats vectors."
