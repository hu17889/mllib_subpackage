...
/**
 * :: DeveloperApi ::
 * A given LambdaTreePoint would belong to a particular node per tree.
...
@DeveloperApi
 class LambdaNodeIdCache(
  ...
  def updateNodeIndices(
      data: RDD[BaggedPoint[LambdaTreePoint]],
...
object LambdaNodeIdCache {
  ...
  def init(
      ...
      initVal: Int = 1): LambdaNodeIdCache = {
    new LambdaNodeIdCache(
