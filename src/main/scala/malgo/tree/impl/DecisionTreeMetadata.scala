 ...
 */
private[spark] class LambdaDecisionTreeMetadata(
    val numFeatures: Int,
...
private[spark] object LambdaDecisionTreeMetadata extends Logging {

  /**
   * Construct a [[LambdaDecisionTreeMetadata]] instance for this dataset and parameters.
  ...
  def buildMetadata(
      ...
      numTrees: Int,
      featureSubsetStrategy: String): LambdaDecisionTreeMetadata = {
  ...
  val numFeatures = input.take(1).headOption.get.features.size
  val numExamples = input.count()
    ...
    // This needs to be checked here instead of in LambdaStrategy since maxPossibleBins can be modified
    ...
    new LambdaDecisionTreeMetadata(numFeatures, numExamples, numClasses, numBins.max,
  ...
  /**
   * Version of [[LambdaDecisionTreeMetadata#buildMetadata]] for DecisionTree.
   */
  def buildMetadata(
      input: RDD[LambdaLabeledPoint],
      strategy: LambdaStrategy): LambdaDecisionTreeMetadata = {
