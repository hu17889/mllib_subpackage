...
@Since("1.2.0")
@Experimental
class LambdaRandomForestModel @Since("1.2.0") (
                                          @Since("1.2.0") override val algo: Algo,
                                          @Since("1.2.0") override val trees: Array[LambdaDecisionTreeModel])
  extends LambdaTreeEnsembleModel(algo, trees, Array.fill(trees.length)(1.0),
  ...
  override def save(sc: SparkContext, path: String): Unit = {
    LambdaTreeEnsembleModel.SaveLoadV1_0.save(sc, path, this,
      LambdaRandomForestModel.SaveLoadV1_0.thisClassName)
  }

  override protected def formatVersion: String = LambdaRandomForestModel.formatVersion
}

@Since("1.3.0")
object LambdaRandomForestModel extends Loader[LambdaRandomForestModel] {

  private[mllib] def formatVersion: String = LambdaTreeEnsembleModel.SaveLoadV1_0.thisFormatVersion
   ...
   */
  @Since("1.3.0")
  override def load(sc: SparkContext, path: String): LambdaRandomForestModel = {
    val (loadedClassName, version, jsonMetadata) = Loader.loadMetadata(sc, path)
    val classNameV1_0 = SaveLoadV1_0.thisClassName
    (loadedClassName, version) match {
      case (className, "1.0") if className == classNameV1_0 =>
        val metadata = LambdaTreeEnsembleModel.SaveLoadV1_0.readMetadata(jsonMetadata)
        assert(metadata.treeWeights.forall(_ == 1.0))
        val trees =
          LambdaTreeEnsembleModel.SaveLoadV1_0.loadTrees(sc, path, metadata.treeAlgo)
        new LambdaRandomForestModel(Algo.fromString(metadata.algo), trees)
      case _ => throw new Exception(s"LambdaRandomForestModel.load did not recognize model" +
  ...
  private object SaveLoadV1_0 {
    ...
    def thisClassName: String = "org.apache.spark.mllib.tree.model.LambdaRandomForestModel"
  }
...
@Since("1.2.0")
@Experimental
class LambdaGradientBoostedTreesModel @Since("1.2.0") (
                                                  @Since("1.2.0") override val algo: Algo,
                                                  @Since("1.2.0") override val trees: Array[LambdaDecisionTreeModel],
                                                  @Since("1.2.0") override val treeWeights: Array[Double])
  extends LambdaTreeEnsembleModel(algo, trees, treeWeights, combiningStrategy = Sum)
  with Saveable {
  ...
  override def save(sc: SparkContext, path: String): Unit = {
    LambdaTreeEnsembleModel.SaveLoadV1_0.save(sc, path, this,
      LambdaGradientBoostedTreesModel.SaveLoadV1_0.thisClassName)
  }
  ...
  def evaluateEachIteration(
    ...
    var predictionAndError = LambdaGradientBoostedTreesModel.computeInitialPredictionAndError(
      remappedData, localTreeWeights(0), trees(0), loss)

    evaluationArray(0) = predictionAndError.values.values.mean()

    val broadcastTrees = sc.broadcast(trees)
    (1 until numIterations).foreach { nTree =>
      predictionAndError = remappedData.map{case x=>((x.qid,x.pos),x)}.join(predictionAndError).map { case (key,(point,(pred,error))) =>
        ...
        (key,(newPred, newError))
        //}
      }
      evaluationArray(nTree) = predictionAndError.values.values.mean()
    }
  ...
  override protected def formatVersion: String = LambdaGradientBoostedTreesModel.formatVersion
}

/**
  */
@Since("1.3.0")
object LambdaGradientBoostedTreesModel extends Loader[LambdaGradientBoostedTreesModel] {
   ...
   * @param initTree: first LambdaDecisionTreeModel.
   * @param loss: evaluation metric.
   ...
   */
  @Since("1.3.0")
  override def load(sc: SparkContext, path: String): LambdaGradientBoostedTreesModel = {
    val (loadedClassName, version, jsonMetadata) = Loader.loadMetadata(sc, path)
    val classNameV1_0 = SaveLoadV1_0.thisClassName
    (loadedClassName, version) match {
      case (className, "1.0") if className == classNameV1_0 =>
        ...
        val trees =
          LambdaTreeEnsembleModel.SaveLoadV1_0.loadTrees(sc, path, metadata.treeAlgo)
        new LambdaGradientBoostedTreesModel(Algo.fromString(metadata.algo), trees, metadata.treeWeights)
      case _ => throw new Exception(s"LambdaGradientBoostedTreesModel.load did not recognize model" +
        s" with (className, format version): ($loadedClassName, $version).  Supported:\n" +
        s"  ($classNameV1_0, 1.0)")
    }
  }
  ...
  private object SaveLoadV1_0 {
    // Hard-code class name string in case it changes in the future
    def thisClassName: String = "org.apache.spark.mllib.tree.model.LambdaGradientBoostedTreesModel"
  }
 ...
 */
 class LambdaTreeEnsembleModel(
                                              protected val algo: Algo,
                                              protected val trees: Array[LambdaDecisionTreeModel],
                                              protected val treeWeights: Array[Double],
                                              protected val combiningStrategy: EnsembleCombiningStrategy) extends Serializable {

  require(numTrees > 0, "LambdaTreeEnsembleModel cannot be created without trees.")
    ...
    (algo, combiningStrategy) match {
      ...
      case _ =>
        throw new IllegalArgumentException(
          "LambdaTreeEnsembleModel given unsupported (algo, combiningStrategy) combination: " +
  ...
  override def toString: String = {
    algo match {
      case Classification =>
        s"LambdaTreeEnsembleModel classifier with $numTrees trees\n"
      case Regression =>
        s"LambdaTreeEnsembleModel regressor with $numTrees trees\n"
      case _ => throw new IllegalArgumentException(
        s"LambdaTreeEnsembleModel given unknown algo parameter: $algo.")
    }
  }
...
private[tree] object LambdaTreeEnsembleModel extends Logging {

  object SaveLoadV1_0 {

    import org.apache.spark.mllib.tree.model.LambdaDecisionTreeModel.SaveLoadV1_0.{NodeData, constructTrees}
     ...
     *  where NodeData is from LambdaDecisionTreeModel.
     */
    case class EnsembleNodeData(treeId: Int, node: NodeData)

    def save(sc: SparkContext, path: String, model: LambdaTreeEnsembleModel, className: String): Unit = {
    ...
    def loadTrees(
                   sc: SparkContext,
                   path: String,
                   treeAlgo: String): Array[LambdaDecisionTreeModel] = {
      val datapath = Loader.dataPath(path)
      val sqlContext = new SQLContext(sc)
      val nodes = sqlContext.read.parquet(datapath).map(NodeData.apply)
      val trees = constructTrees(nodes)
      trees.map(new LambdaDecisionTreeModel(_, Algo.fromString(treeAlgo)))
    }
  }
