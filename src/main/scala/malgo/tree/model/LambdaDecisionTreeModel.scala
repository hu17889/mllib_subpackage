...
@Since("1.0.0")
@Experimental
class LambdaDecisionTreeModel @Since("1.0.0") (
    @Since("1.0.0") val topNode: MNode,
  ...
  override def toString: String = algo match {
    case Classification =>
      s"LambdaDecisionTreeModel classifier of depth $depth with $numNodes nodes"
    case Regression =>
      s"LambdaDecisionTreeModel regressor of depth $depth with $numNodes nodes"
    case _ => throw new IllegalArgumentException(
      s"LambdaDecisionTreeModel given unknown algo parameter: $algo.")
  }
  ...
  override def save(sc: SparkContext, path: String): Unit = {
    LambdaDecisionTreeModel.SaveLoadV1_0.save(sc, path, this)
  }

  override protected def formatVersion: String = LambdaDecisionTreeModel.formatVersion
}

@Since("1.3.0")
object LambdaDecisionTreeModel extends Loader[LambdaDecisionTreeModel] with Logging {
  ...
  private[tree] object SaveLoadV1_0 {
    ...
    def thisClassName: String = "org.apache.spark.mllib.tree.LambdaDecisionTreeModel"
    ...
    object NodeData {
      def apply(treeId: Int, n: MNode): NodeData = {
    ...
    def save(sc: SparkContext, path: String, model: LambdaDecisionTreeModel): Unit = {
    ...
    def load(sc: SparkContext, path: String, algo: String, numNodes: Int): LambdaDecisionTreeModel = {
      ...
      val model = new LambdaDecisionTreeModel(trees(0), Algo.fromString(algo))
      assert(model.numNodes == numNodes, s"Unable to load LambdaDecisionTreeModel data from: $datapath." +
        s" Expected $numNodes nodes but found ${model.numNodes}")
      model
    }

    def constructTrees(nodes: RDD[NodeData]): Array[MNode] = {
      val trees = nodes
        .groupBy(_.treeId)
        .mapValues(_.toArray)
        .collect()
     ...
     */
    def constructTree(data: Array[NodeData]): MNode = {
      val dataMap: Map[Int, NodeData] = data.map(n => n.nodeId -> n).toMap
      assert(dataMap.contains(1),
        s"LambdaDecisionTree missing root node (id = 1).")
    ...
    private def constructNode(
      ...
      nodes: mutable.Map[Int, MNode]): MNode = {
      ...
      val node =
        if (data.isLeaf) {
          MNode(data.nodeId, data.predict.toPredict, data.impurity, data.isLeaf)
        } else {
          ...
          new MNode(data.nodeId, data.predict.toPredict, data.impurity, data.isLeaf,
   ...
   */
  @Since("1.3.0")
  override def load(sc: SparkContext, path: String): LambdaDecisionTreeModel = {
    ...
    (loadedClassName, version) match {
      ...
      case _ => throw new Exception(
        s"LambdaDecisionTreeModel.load did not recognize model with (className, format version):" +
