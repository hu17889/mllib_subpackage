...
class MNode @Since("1.2.0") (
    ...
    @Since("1.0.0") var leftNode: Option[MNode],
    @Since("1.0.0") var rightNode: Option[MNode],
  ...
  @deprecated("build should no longer be used since trees are constructed on-the-fly in training",
    "1.2.0")
  def build(nodes: Array[MNode]): Unit = {
    logDebug("building node " + id + " at level " + MNode.indexToLevel(id))
    logDebug("id = " + id + ", split = " + split)
    logDebug("stats = " + stats)
    logDebug("predict = " + predict)
    logDebug("impurity = " + impurity)
    if (!isLeaf) {
      leftNode = Some(nodes(MNode.leftChildIndex(id)))
      rightNode = Some(nodes(MNode.rightChildIndex(id)))
   ...
   */
  private[tree] def deepCopy(): MNode = {
    ...
    new MNode(id, predict, impurity, isLeaf, split, leftNodeCopy, rightNodeCopy, stats)
  }
  ...
  private[tree] def subtreeIterator: Iterator[MNode] = {
...
object MNode {
   ...
   */
  def emptyNode(nodeIndex: Int): MNode = new MNode(nodeIndex, new Predict(Double.MinValue), -1.0,
  ...
  def apply(
      ...
      predict: Predict,
      impurity: Double,
      isLeaf: Boolean): MNode = {
    new MNode(nodeIndex, predict, impurity, isLeaf, None, None, None, None)
  }
   ...
   */
  def getNode(nodeIndex: Int, rootNode: MNode): MNode = {
    var tmpNode: MNode = rootNode
