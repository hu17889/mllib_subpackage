...
import org.apache.spark.mllib.tree.impl.{BaggedPoint, LambdaDecisionTreeMetadata, LambdaNodeIdCache,
  TimeTracker, LambdaTreePoint}
 ...
 */
@Experimental
private class LambdaRandomForest (
    private val strategy: LambdaStrategy,
  ...
  require(numTrees > 0, s"LambdaRandomForest requires numTrees > 0, but was given numTrees = $numTrees.")
  require(LambdaRandomForest.supportedFeatureSubsetStrategies.contains(featureSubsetStrategy),
    s"LambdaRandomForest given invalid featureSubsetStrategy: $featureSubsetStrategy." +
    s" Supported values: ${LambdaRandomForest.supportedFeatureSubsetStrategies.mkString(", ")}.")
   ...
   */
  def run(input: RDD[LambdaLabeledPoint]): LambdaRandomForestModel = {
    ...
    val retaggedInput = input.retag(classOf[LambdaLabeledPoint])
    val metadata =
      LambdaDecisionTreeMetadata.buildMetadata(retaggedInput, strategy, numTrees, featureSubsetStrategy)
    ...
    val (splits, bins) = LambdaDecisionTree.findSplitsBins(retaggedInput, metadata)
    ...
    // Bin feature values (LambdaTreePoint representation).
    // Cache input RDD for speedup during multiple passes.
    val treeInput = LambdaTreePoint.convertToTreeRDD(retaggedInput, bins, metadata)
    ...
    val maxMemoryPerNode = {
      ...
      LambdaRandomForest.aggregateSizeForNode(metadata, featureSubset) * 8L
    }
    require(maxMemoryPerNode <= maxMemoryUsage,
      s"LambdaRandomForest/DecisionTree given maxMemoryInMB = ${strategy.maxMemoryInMB}," +
    ...
    val nodeQueue = new mutable.Queue[(Int, MNode)]()

    val rng = new scala.util.Random()
    rng.setSeed(seed)

    // Allocate and queue root nodes.
    val topNodes: Array[MNode] = Array.fill[MNode](numTrees)(MNode.emptyNode(nodeIndex = 1))
    ...
    while (nodeQueue.nonEmpty) {
      ...
      val (nodesForGroup, treeToNodeToIndexInfo) =
        LambdaRandomForest.selectNodesToSplit(nodeQueue, maxMemoryUsage, metadata, rng)
      // Sanity check (should never occur):
      assert(nodesForGroup.size > 0,
        s"LambdaRandomForest selected empty nodesForGroup.  Error for unknown reason.")

      // Choose node splits, and enqueue new nodes as needed.
      timer.start("findBestSplits")
      LambdaDecisionTree.findBestSplits(baggedInput, metadata, topNodes, nodesForGroup,
        treeToNodeToIndexInfo, splits, bins, nodeQueue, timer, nodeIdCache)
    ...
    val trees = topNodes.map(topNode => new LambdaDecisionTreeModel(topNode, strategy.algo))
    new LambdaRandomForestModel(strategy.algo, trees)
  }

}

@Since("1.2.0")
object LambdaRandomForest extends Serializable with Logging {
  ...
  def trainClassifier(
      ...
      numTrees: Int,
      featureSubsetStrategy: String,
      seed: Int): LambdaRandomForestModel = {
    require(strategy.algo == Classification,
      s"LambdaRandomForest.trainClassifier given LambdaStrategy with invalid algo: ${strategy.algo}")
    val rf = new LambdaRandomForest(strategy, numTrees, featureSubsetStrategy, seed)
    rf.run(input)
  }
  ...
  def trainClassifier(
      ...
      maxDepth: Int,
      maxBins: Int,
      seed: Int = Utils.random.nextInt()): LambdaRandomForestModel = {
    val impurityType = Impurities.fromString(impurity)
    val strategy = new LambdaStrategy(Classification, impurityType, maxDepth,
  ...
  def trainRegressor(
    ...
    require(strategy.algo == Regression,
      s"LambdaRandomForest.trainRegressor given LambdaStrategy with invalid algo: ${strategy.algo}")
    val rf = new LambdaRandomForest(strategy, numTrees, featureSubsetStrategy, seed)
    rf.run(input)
  }
  ...
  def trainRegressor(
      ...
      seed: Int = Utils.random.nextInt()): LambdaRandomForestModel = {
    val impurityType = Impurities.fromString(impurity)
    val strategy = new LambdaStrategy(Regression, impurityType, maxDepth,
  ...
  private[tree] def selectNodesToSplit(
      ...
      rng: scala.util.Random): (Map[Int, Array[MNode]], Map[Int, Map[Int, NodeIndexInfo]]) = {
    // Collect some nodes to split:
    //  nodesForGroup(treeIndex) = nodes to split
    val mutableNodesForGroup = new mutable.HashMap[Int, mutable.ArrayBuffer[MNode]]()
    ...
    while (nodeQueue.nonEmpty && memUsage < maxMemoryUsage) {
      ...
      val nodeMemUsage = LambdaRandomForest.aggregateSizeForNode(metadata, featureSubset) * 8L
      if (memUsage + nodeMemUsage <= maxMemoryUsage) {
        nodeQueue.dequeue()
        mutableNodesForGroup.getOrElseUpdate(treeIndex, new mutable.ArrayBuffer[MNode]()) += node
    ...
    val nodesForGroup: Map[Int, Array[MNode]] = mutableNodesForGroup.mapValues(_.toArray).toMap
